...
```{r averagePerInterval, echo=TRUE}
...
```{r averagePerIntervalPlot, echo=TRUE}
...
```{r stepaMax, echo=TRUE,}
stepsMax = averagePerInterval[which.max(averagePerInterval$Steps),]
```

At `r stepsMax$Interval` hours on average, during the day the subject took 
`r stepsMax$Steps` steps, which is the, on average, the highest number of steps
druing the day.
...
```{r nasum, echo=TRUE, results='asis'}
sum(is.na(df$Steps))
```
...
```{r dfNoNA, echo=TRUE, results='asis'}
...
```{r nonameanmedia, echo=TRUE}
stepsMeanPerDayNoNA = mean(stepsPerDayNoNA$Steps)
stepsMedianPerDayNoNA = median(stepsPerDayNoNA$Steps)
```

*Mean:* `r  as.character(stepsMeanPerDayNoNA)`

*Median:* `r as.character(stepsMedianPerDayNoNA)`
...
```{r averagePerIntervalOnWeekend, echo=TRUE}
...
And for weekday.
```{r averagePerIntervalOnWeekday, echo=TRUE}
...
```{r weekplotsrc, echo=TRUE, results='hide'}
with(df, {
